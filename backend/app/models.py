...
class Document(DocumentBase, table=True):
    ...
    topics: list["Topic"] | None = Relationship(
        back_populates="document", link_model=DocumentTopicLink
    )
...
class Topic(TopicBase, table=True):
    id: uuid.UUID = Field(default_factory=uuid.uuid4, primary_key=True)
    document: list["Document"] = Relationship(
        back_populates="topics", link_model=DocumentTopicLink
    )
