...
def cleanDataTier2(
    data, stopword_df, language, min_len=1, delete_list=[]
):  # removing unnecessary words
    stopword_list = []
    try:
        stopword_list = stopword_df[stopword_df["Language"] == language][
            "Stopword_list"
        ].values[0]
    except:
        return None
    ...
    if type(data) == str:
        try:
            ...
            for word in words:  # iterate over all words from the text
                if (
                    word not in stopword_list and len(word) > min_len
                ):  # only add words that are not in the French stopwords list, and are more than 1 character
        ...
        try:
            ...
            for word in words:
                stemmed_word = stemmer.stem(
                    stemmer.stem(stemmer.stem(word))
                )  # stem the word three times to be sure
            ...
            for element in data:
                ...
                for word in words:
                    ...
                    stemmed_words.append(
                        stemmed_word
                    )  # add it to our stemmed word list
...
class Miner:
    ...
    def mine(self, doc_df, content_column_name="content"):
        ...
        without_error_df["language"] = without_error_df.apply(
            lambda row: detect_language(row[content_column_name]), axis=1
        )
        # strip doc_df['language']== None and set doc_df['error'] to 'Cannot detect language' accordingly
        tmp_without_error_df = deepcopy(
            without_error_df[without_error_df["language"].notnull()]
        )
        tmp_with_error_df = deepcopy(
            without_error_df[without_error_df["language"].isnull()]
        )
        ...
        without_error_df["stripped"] = without_error_df.apply(
            lambda row: normalize(row[content_column_name]), axis=1
        )
        tmp_without_error_df = deepcopy(
            without_error_df[without_error_df["stripped"].notnull()]
        )
        tmp_with_error_df = deepcopy(
            without_error_df[without_error_df["stripped"].isnull()]
        )
        ...
        without_error_df["lemmatized"] = without_error_df.apply(
            lambda row: self.lemmatize(row["stripped"]), axis=1
        )
        tmp_without_error_df = deepcopy(
            without_error_df[without_error_df["lemmatized"].notnull()]
        )
        tmp_with_error_df = deepcopy(
            without_error_df[without_error_df["lemmatized"].isnull()]
        )
        ...
        without_error_df["without_stop_words"] = without_error_df.apply(
            lambda row: self.delete_stop_words(row["language"], row["lemmatized"]),
            axis=1,
        )
        tmp_without_error_df = deepcopy(
            without_error_df[without_error_df["without_stop_words"].notnull()]
        )
        tmp_with_error_df = deepcopy(
            without_error_df[without_error_df["without_stop_words"].isnull()]
        )
        ...
        if with_error_df.empty:
            return without_error_df.sort_values("original_order").drop(
                "original_order", axis=1
            )
        else:
            return (
                pd.concat([with_error_df, without_error_df], axis=0)
                .sort_values("original_order")
                .drop("original_order", axis=1)
            )
