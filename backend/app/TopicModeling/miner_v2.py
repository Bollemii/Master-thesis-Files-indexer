...
def detect_language(text: str) -> Optional[str]:
    ...
    try:
        ...
        return next(
            (item.lang.upper() for item in res if item.lang in ("fr", "en")), None
        )
...
class Miner:
    ...
    def lemmatize(self, text: str) -> str:
        ...
        return " ".join(
            self.lemmatizer.lemmatize(word) for word in self.word_split.split(text)
        )
    ...
    def mine(
        ...
        result_df.loc[mask, "language"] = valid_docs[content_column_name].apply(
            detect_language
        )
        lang_mask = result_df["language"].notnull()

        result_df.loc[mask & lang_mask, "stripped"] = valid_docs[content_column_name][
            lang_mask
        ].apply(normalize)
        strip_mask = result_df["stripped"].notnull()

        result_df.loc[mask & lang_mask & strip_mask, "lemmatized"] = result_df[
            "stripped"
        ][mask & lang_mask & strip_mask].apply(self.lemmatize)
        lem_mask = result_df["lemmatized"].notnull()

        result_df.loc[
            mask & lang_mask & strip_mask & lem_mask, "without_stop_words"
        ...
        result_df.loc[mask & lang_mask & strip_mask & ~lem_mask, "error"] = (
