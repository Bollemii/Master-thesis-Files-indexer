...
class Reader:
            ...
            for i_task in task_map.keys():
                ...
                if type(t) is not float:
                    ...
                    if dt > task_timeout * (1 + n_pages_map[i_task]):
                        ...
                        os.kill(
                            pid, signal.SIGTERM
                        )  # or signal.SIGKILL (linux only) # process continues processing but pipe is broken => not possible to receive any update => safe
    ...
    def modified_read(
        self, doc_df
    ):  # Note that due to GIL, there's no real full potential multi-threading in cPython :( => we prefer using Process!!! even if we have overhead for very simple file (as text, doc, docs,... without OCR)
        #
