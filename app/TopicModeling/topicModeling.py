...
#        doc_df = doc_df.head(120)
    ...
    if 1 == 0:
        ...
        if os.path.isfile(result_file):
            ...
            diff_meta_df = pd.concat([meta_df, old_meta_df, old_meta_df]).drop_duplicates(keep=False)
        else:
    ...
    if 1 == 0:
        ...
        if diff_meta_df.empty:
            ...
            diff_doc_df = doc_df.join(diff_meta_df.set_index(['file_name', 'creation_time', 'file_size']), on=['file_name', 'creation_time', 'file_size'], how='inner')

            diff_doc_df['raw_content'] = diff_doc_df['content']
            diff_doc_df['content'] = diff_doc_df.apply(lambda row: delete_eol(row['raw_content']), axis=1)
            diff_doc_df['file_type'] = (diff_doc_df.apply(lambda row: pathlib.PurePosixPath(row['file_path'])
                                                          .suffix.replace('.', ''), axis=1))
            diff_doc_df = diff_doc_df.drop(columns=['file_path'])
            diff_doc_df['content_size'] = diff_doc_df.apply(lambda row: get_length(row['content']), axis=1)
            diff_doc_df['raw_content_size'] = diff_doc_df.apply(lambda row: get_length(row['raw_content']), axis=1)
            diff_doc_df = diff_doc_df[['file_name', 'file_type', 'creation_time', 'file_size', 'n_pages', 'dt', 'error',
                                       'content_size', 'content', 'raw_content_size', 'raw_content']]
            print(diff_doc_df)
